 ...
 */
#define LACE_TYPEDEF_CB(t, f, ...) \
	typedef t (*f)(WorkerP *, Task *, ##__VA_ARGS__);
...
void lace_run_task_without_block(Task *task);
 ...
 */
#define WRAP(f, ...) \
	(f((WorkerP *)__lace_worker, (Task *)__lace_dq_head, ##__VA_ARGS__))
...
#define RUN_WITHOUT_BLOCK(f, ...) (f##_RUN_WITHOUT_BLOCK(__VA_ARGS__))
 ...
 */
#define LACE_VARS                                                           \
	WorkerP *__attribute__((unused)) __lace_worker = lace_get_worker(); \
	Task *__attribute__((unused))                                       \
...
#define YIELD_NEWFRAME()                                                   \
	{                                                                  \
		if (unlikely(atomic_load_explicit(&lace_newframe.t,        \
...
#define LACE_TRNG             \
	(__lace_worker->rng = \
		 2862933555777941757ULL * __lace_worker->rng + 3037000493ULL)
...
#define LACE_COUNT_EVENTS                                           \
	(LACE_PIE_TIMES || LACE_COUNT_TASKS || LACE_COUNT_STEALS || \
	 LACE_COUNT_SPLITS)
#endif
...
struct __lace_common_fields_only {
...
typedef struct _Task {
	...
	char p2[PAD(ROUND(LACE_COMMON_FIELD_SIZE, P_SZ) + LACE_TASKSIZE,
		    LINE_SIZE)];
} Task;
 ...
 */
#define LACE_MAKE_ALL_SHARED() \
	lace_make_all_shared(__lace_worker, __lace_dq_head)
...
{
	uint64_t now = gethrtime(), prev = w->time;
...
lace_steal(WorkerP *self, Task *__dq_head, Worker *victim)
{
	if (victim != NULL && !victim->allstolen) {
		...
		if (ts.ts.tail < ts.ts.split) {
			...
			ts_new.v = ts.v;
			ts_new.ts.tail++;
			if (atomic_compare_exchange_weak(&victim->ts.v, &ts.v,
							 ts_new.v)) {
				// Stolen
				Task *t = &victim->dq[ts.ts.tail];
				atomic_store_explicit(&t->thief, self->_public,
						      memory_order_relaxed);
				lace_time_event(self, 1);
				t->f(self, __dq_head, t);
				lace_time_event(self, 2);
				atomic_store_explicit(&t->thief,
						      THIEF_COMPLETED,
...
static int lace_shrink_shared(WorkerP *w)
{
	Worker *wt = w->_public;
	...
	if (tail != split) {
		...
		atomic_store_explicit(
			...
			memory_order_relaxed); /* emit normal write */
...
static inline void lace_leapfrog(WorkerP *__lace_worker, Task *__lace_dq_head)
{
	...
	if (thief != THIEF_COMPLETED) {
		...
		while (thief != THIEF_COMPLETED) {
			...
			Worker *res = lace_steal(__lace_worker, __lace_dq_head,
						 thief);
			if (res == LACE_NOWORK) {
				YIELD_NEWFRAME();
...
static __attribute__((noinline)) void lace_drop_slow(WorkerP *w,
						     Task *__dq_head)
{
...
static inline __attribute__((unused)) void lace_drop(WorkerP *w,
						     Task *__dq_head)
{
...
#define TASK_DECL_0(RTYPE, NAME)                                                                              \
	...
	typedef char assertion_failed_task_descriptor_out_of_bounds_##NAME                                    \
		[(sizeof(TD_##NAME) <= sizeof(Task)) ? 0 : -1];                                               \
                                                                                                              \
	...
	static RTYPE NAME##_SYNC_SLOW(WorkerP *, Task *);                                                     \
                                                                                                              \
	static inline __attribute__((unused)) void NAME##_SPAWN(                                              \
		WorkerP *w, Task *__dq_head)                                                                  \
	{                                                                                                     \
		PR_COUNTTASK(w);                                                                              \
                                                                                                              \
		TD_##NAME *t;                                                                                 \
		...
		t->f = &NAME##_WRAP;                                                                          \
		atomic_store_explicit(&t->thief, THIEF_TASK,                                                  \
				      memory_order_relaxed);                                                  \
                                                                                                              \
		/*compiler_barrier();*/                                                                       \
		atomic_thread_fence(memory_order_acquire);                                                    \
		...
		if (unlikely(w->allstolen)) {                                                                 \
			...
			PR_COUNTSPLITS(w, CTR_split_grow);                                                    \
                                                                                                              ...
                                                                                                              \
	static inline __attribute__((unused)) RTYPE NAME##_NEWFRAME()                                         \
	{                                                                                                     \
		Task _t;                                                                                      \
		...
		return ((TD_##NAME *)t)->d.res;                                                               \
                                                                                                              ...
                                                                                                              \
	static inline __attribute__((unused)) void NAME##_TOGETHER()                                          \
	{                                                                                                     \
		Task _t;                                                                                      \
		TD_##NAME *t = (TD_##NAME *)&_t;                                                              \
		t->f = &NAME##_WRAP;                                                                          \
		...
		lace_run_together(&_t);                                                                       \
                                                                                                              ...
                                                                                                              \
	static inline __attribute__((unused)) RTYPE NAME##_RUN()                                              \
	{                                                                                                     \
		Task _t;                                                                                      \
		TD_##NAME *t = (TD_##NAME *)&_t;                                                              \
		t->f = &NAME##_WRAP;                                                                          \
	...
	{                                                                                                     \
		TD_##NAME *t;                                                                                 \
                                                                                                              \
		if ((w->allstolen) ||                                                                         \
		    (w->split > __dq_head && lace_shrink_shared(w))) {                                        \
			lace_leapfrog(w, __dq_head);                                                          \
			t = (TD_##NAME *)__dq_head;                                                           \
			return ((TD_##NAME *)t)->d.res;                                                       \
		}                                                                                             \
		...
		/*compiler_barrier();*/                                                                       \
                                                                                                              \
		t = (TD_##NAME *)__dq_head;                                                                   \
		atomic_store_explicit(&t->thief, THIEF_EMPTY,                                                 \
				      memory_order_relaxed);                                                  \
...
#define TASK_IMPL_0(RTYPE, NAME)                                                                    \
	void NAME##_WRAP(WorkerP *w, Task *__dq_head,                                               \
			 TD_##NAME *t __attribute__((unused)))                                      \
	{                                                                                           \
	...
	{                                                                                           \
		return NAME##_WORK(w, __dq_head);                                                   \
                                                                                                    ...
                                                                                                    \
	static inline __attribute__((always_inline))                                                \
	RTYPE NAME##_WORK(WorkerP *__lace_worker __attribute__((unused)),                           \
			  Task *__lace_dq_head __attribute__((unused)))
...
#define VOID_TASK_DECL_0(NAME)                                                                                \
	...
	{                                                                                                     \
		...
		if (unlikely(w->allstolen)) {                                                                 \
			...
			PR_COUNTSPLITS(w, CTR_split_grow);                                                    \
                                                                                                              ...
                                                                                                              \
	static inline __attribute__((unused)) void NAME##_NEWFRAME()                                          \
	{                                                                                                     \
		Task _t;                                                                                      \
		TD_##NAME *t = (TD_##NAME *)&_t;                                                              \
		t->f = &NAME##_WRAP;                                                                          \
	...
	{                                                                                                     \
		...
		lace_run_together(&_t);                                                                       \
                                                                                                              ...
                                                                                                              \
	static inline __attribute__((unused)) void NAME##_RUN()                                               \
	{                                                                                                     \
		Task _t;                                                                                      \
		TD_##NAME *t = (TD_##NAME *)&_t;                                                              \
		t->f = &NAME##_WRAP;                                                                          \
		...
		return;                                                                                       \
                                                                                                              ...
                                                                                                              \
	static __attribute__((noinline)) void NAME##_SYNC_SLOW(                                               \
		WorkerP *w, Task *__dq_head)                                                                  \
	{                                                                                                     \
		TD_##NAME *t;                                                                                 \
                                                                                                              \
		...
		return NAME##_CALL(w, __dq_head);                                                             \
                                                                                                              ...
                                                                                                              \
	static inline __attribute__((unused)) void NAME##_SYNC(                                               \
		WorkerP *w, Task *__dq_head)                                                                  \
	{                                                                                                     \
		/* assert (__dq_head > 0); */ /* Commented out because we assume contract */                  \
                                                                                                              \
...
#define VOID_TASK_IMPL_0(NAME)                                                                      \
	...
	{                                                                                           \
		NAME##_CALL(w, __dq_head);                                                          \
                                                                                                    ...
                                                                                                    \
	static inline __attribute__((always_inline)) void NAME##_WORK(                              \
	...
	{                                                                                           \
		return NAME##_WORK(w, __dq_head);                                                   \
                                                                                                    ...
                                                                                                    \
	static inline __attribute__((always_inline)) void NAME##_WORK(                              \
		WorkerP *__lace_worker __attribute__((unused)),                                     \
		Task *__lace_dq_head __attribute__((unused)))
...
#define TASK_DECL_1(RTYPE, NAME, ATYPE_1)                                                                     \
	...
	static RTYPE NAME##_SYNC_SLOW(WorkerP *, Task *);                                                     \
                                                                                                              \
	static inline __attribute__((unused)) void NAME##_SPAWN(                                              \
		WorkerP *w, Task *__dq_head, ATYPE_1 arg_1)                                                   \
	{                                                                                                     \
		PR_COUNTTASK(w);                                                                              \
                                                                                                              \
		TD_##NAME *t;                                                                                 \
	...
	{                                                                                                     \
		...
		return ((TD_##NAME *)t)->d.res;                                                               \
                                                                                                              ...
                                                                                                              \
	static inline                                                                                         \
		__attribute__((unused)) void NAME##_TOGETHER(ATYPE_1 arg_1)                                   \
	{                                                                                                     \
		Task _t;                                                                                      \
		TD_##NAME *t = (TD_##NAME *)&_t;                                                              \
		t->f = &NAME##_WRAP;                                                                          \
		...
		lace_run_together(&_t);                                                                       \
                                                                                                              ...
                                                                                                              \
	static inline __attribute__((unused)) RTYPE NAME##_RUN(ATYPE_1 arg_1)                                 \
	{                                                                                                     \
		Task _t;                                                                                      \
		TD_##NAME *t = (TD_##NAME *)&_t;                                                              \
		t->f = &NAME##_WRAP;                                                                          \
...
#define TASK_IMPL_1(RTYPE, NAME, ATYPE_1, ARG_1)                                                    \
	...
	{                                                                                           \
		t->d.res = NAME##_CALL(w, __dq_head, t->d.args.arg_1);                              \
                                                                                                    ...
                                                                                                    \
	static inline __attribute__((always_inline)) RTYPE NAME##_WORK(                             \
		WorkerP *__lace_worker, Task *__lace_dq_head, ATYPE_1);                             \
                                                                                                    \
	/* NAME##_WORK is inlined in NAME##_CALL and the parameter __lace_in_task will disappear */ \
...
#define VOID_TASK_DECL_1(NAME, ATYPE_1)                                                                       \
	...
	{                                                                                                     \
		...
		if (unlikely(w->allstolen)) {                                                                 \
			...
			PR_COUNTSPLITS(w, CTR_split_grow);                                                    \
                                                                                                              ...
                                                                                                              \
	static inline                                                                                         \
		__attribute__((unused)) void NAME##_NEWFRAME(ATYPE_1 arg_1)                                   \
	{                                                                                                     \
		Task _t;                                                                                      \
		TD_##NAME *t = (TD_##NAME *)&_t;                                                              \
		t->f = &NAME##_WRAP;                                                                          \
	...
	{                                                                                                     \
		...
		lace_run_together(&_t);                                                                       \
                                                                                                              ...
                                                                                                              \
	static inline __attribute__((unused)) void NAME##_RUN(ATYPE_1 arg_1)                                  \
	{                                                                                                     \
		Task _t;                                                                                      \
		TD_##NAME *t = (TD_##NAME *)&_t;                                                              \
		t->f = &NAME##_WRAP;                                                                          \
...
#define VOID_TASK_IMPL_1(NAME, ATYPE_1, ARG_1)                                                      \
	...
	static inline __attribute__((always_inline)) void NAME##_WORK(                              \
		WorkerP *__lace_worker __attribute__((unused)),                                     \
		Task *__lace_dq_head __attribute__((unused)), ATYPE_1 ARG_1)

#define VOID_TASK_1(NAME, ATYPE_1, ARG_1) \
	VOID_TASK_DECL_1(NAME, ATYPE_1) VOID_TASK_IMPL_1(NAME, ATYPE_1, ARG_1)
...
#define TASK_DECL_2(RTYPE, NAME, ATYPE_1, ATYPE_2)                                                            \
	...
	static RTYPE NAME##_SYNC_SLOW(WorkerP *, Task *);                                                     \
                                                                                                              \
	static inline __attribute__((unused)) void NAME##_SPAWN(                                              \
		WorkerP *w, Task *__dq_head, ATYPE_1 arg_1, ATYPE_2 arg_2)                                    \
	{                                                                                                     \
		PR_COUNTTASK(w);                                                                              \
                                                                                                              \
		TD_##NAME *t;                                                                                 \
	...
	{                                                                                                     \
		...
		return ((TD_##NAME *)t)->d.res;                                                               \
                                                                                                              ...
                                                                                                              \
	static inline __attribute__((unused))                                                                 \
	RTYPE NAME##_RUN_WITHOUT_BLOCK(ATYPE_1 arg_1, ATYPE_2 arg_2)                                          \
	{                                                                                                     \
		Task _t;                                                                                      \
		TD_##NAME *t = (TD_##NAME *)&_t;                                                              \
		t->f = &NAME##_WRAP;                                                                          \
		...
		t->d.args.arg_1 = arg_1;                                                                      \
		t->d.args.arg_2 = arg_2;                                                                      \
		lace_run_task_without_block(&_t);                                                             \
		return ((TD_##NAME *)t)->d.res;                                                               \
	}                                                                                                     \
                                                                                                              \
	static __attribute__((noinline))                                                                      \
...
#define TASK_DECL_3(RTYPE, NAME, ATYPE_1, ATYPE_2, ATYPE_3)                                                   \
	...
	{                                                                                                     \
		...
		return ((TD_##NAME *)t)->d.res;                                                               \
                                                                                                              ...
                                                                                                              \
	static inline __attribute__((unused)) void NAME##_TOGETHER(                                           \
		ATYPE_1 arg_1, ATYPE_2 arg_2, ATYPE_3 arg_3)                                                  \
	{                                                                                                     \
		Task _t;                                                                                      \
		TD_##NAME *t = (TD_##NAME *)&_t;                                                              \
		t->f = &NAME##_WRAP;                                                                          \
...
#define TASK_IMPL_3(RTYPE, NAME, ATYPE_1, ARG_1, ATYPE_2, ARG_2, ATYPE_3,                           \
		    ARG_3)                                                                          \
	void NAME##_WRAP(WorkerP *w, Task *__dq_head,                                               \
			 TD_##NAME *t __attribute__((unused)))                                      \
	{                                                                                           \
...
#define TASK_3(RTYPE, NAME, ATYPE_1, ARG_1, ATYPE_2, ARG_2, ATYPE_3, ARG_3) \
	...
	TASK_IMPL_3(RTYPE, NAME, ATYPE_1, ARG_1, ATYPE_2, ARG_2, ATYPE_3,   \
		    ARG_3)
	...
	{                                                                                                     \
		...
		if (unlikely(w->allstolen)) {                                                                 \
			...
			PR_COUNTSPLITS(w, CTR_split_grow);                                                    \
                                                                                                              ...
                                                                                                              \
	static inline __attribute__((unused)) RTYPE NAME##_NEWFRAME(                                          \
		ATYPE_1 arg_1, ATYPE_2 arg_2, ATYPE_3 arg_3, ATYPE_4 arg_4)                                   \
	{                                                                                                     \
		Task _t;                                                                                      \
		TD_##NAME *t = (TD_##NAME *)&_t;                                                              \
		t->f = &NAME##_WRAP;                                                                          \
...
#define TASK_IMPL_4(RTYPE, NAME, ATYPE_1, ARG_1, ATYPE_2, ARG_2, ATYPE_3,                           \
		    ARG_3, ATYPE_4, ARG_4)                                                          \
	void NAME##_WRAP(WorkerP *w, Task *__dq_head,                                               \
			 TD_##NAME *t __attribute__((unused)))                                      \
	{                                                                                           \
	...
	static inline __attribute__((always_inline))                                                \
	RTYPE NAME##_WORK(WorkerP *__lace_worker, Task *__lace_dq_head,                             \
			  ATYPE_1, ATYPE_2, ATYPE_3, ATYPE_4);                                      \
                                                                                                    \
	/* NAME##_WORK is inlined in NAME##_CALL and the parameter __lace_in_task will disappear */ \
	RTYPE NAME##_CALL(WorkerP *w, Task *__dq_head, ATYPE_1 arg_1,                               \
			  ATYPE_2 arg_2, ATYPE_3 arg_3, ATYPE_4 arg_4)                              \
	{                                                                                           \
...
#define TASK_4(RTYPE, NAME, ATYPE_1, ARG_1, ATYPE_2, ARG_2, ATYPE_3, ARG_3, \
	       ATYPE_4, ARG_4)                                              \
	TASK_DECL_4(RTYPE, NAME, ATYPE_1, ATYPE_2, ATYPE_3, ATYPE_4)        \
	TASK_IMPL_4(RTYPE, NAME, ATYPE_1, ARG_1, ATYPE_2, ARG_2, ATYPE_3,   \
		    ARG_3, ATYPE_4, ARG_4)
...
#define VOID_TASK_IMPL_4(NAME, ATYPE_1, ARG_1, ATYPE_2, ARG_2, ATYPE_3, ARG_3,                      \
			 ATYPE_4, ARG_4)                                                            \
	void NAME##_WRAP(WorkerP *w, Task *__dq_head,                                               \
			 TD_##NAME *t __attribute__((unused)))                                      \
	{                                                                                           \
		NAME##_CALL(w, __dq_head, t->d.args.arg_1, t->d.args.arg_2,                         \
			    t->d.args.arg_3, t->d.args.arg_4);                                      \
	}                                                                                           \
                                                                                                    \
	static inline __attribute__((always_inline)) void NAME##_WORK(                              \
		WorkerP *__lace_worker, Task *__lace_dq_head, ATYPE_1,                              \
		ATYPE_2, ATYPE_3, ATYPE_4);                                                         \
                                                                                                    \
	/* NAME##_WORK is inlined in NAME##_CALL and the parameter __lace_in_task will disappear */ \
	void NAME##_CALL(WorkerP *w, Task *__dq_head, ATYPE_1 arg_1,                                \
			 ATYPE_2 arg_2, ATYPE_3 arg_3, ATYPE_4 arg_4)                               \
...
#define VOID_TASK_4(NAME, ATYPE_1, ARG_1, ATYPE_2, ARG_2, ATYPE_3, ARG_3,      \
		    ATYPE_4, ARG_4)                                            \
	VOID_TASK_DECL_4(NAME, ATYPE_1, ATYPE_2, ATYPE_3, ATYPE_4)             \
	VOID_TASK_IMPL_4(NAME, ATYPE_1, ARG_1, ATYPE_2, ARG_2, ATYPE_3, ARG_3, \
			 ATYPE_4, ARG_4)
...
#define TASK_DECL_5(RTYPE, NAME, ATYPE_1, ATYPE_2, ATYPE_3, ATYPE_4, ATYPE_5)                                 \
	...
	void NAME##_WRAP(WorkerP *, Task *, TD_##NAME *);                                                     \
	RTYPE NAME##_CALL(WorkerP *, Task *, ATYPE_1 arg_1, ATYPE_2 arg_2,                                    \
			  ATYPE_3 arg_3, ATYPE_4 arg_4, ATYPE_5 arg_5);                                       \
	static inline RTYPE NAME##_SYNC(WorkerP *, Task *);                                                   \
	static RTYPE NAME##_SYNC_SLOW(WorkerP *, Task *);                                                     \
                                                                                                              \
	static inline __attribute__((unused)) void NAME##_SPAWN(                                              \
...
#define TASK_IMPL_5(RTYPE, NAME, ATYPE_1, ARG_1, ATYPE_2, ARG_2, ATYPE_3,                           \
		    ARG_3, ATYPE_4, ARG_4, ATYPE_5, ARG_5)                                          \
	void NAME##_WRAP(WorkerP *w, Task *__dq_head,                                               \
			 TD_##NAME *t __attribute__((unused)))                                      \
	{                                                                                           \
		t->d.res = NAME##_CALL(w, __dq_head, t->d.args.arg_1,                               \
	...
	static inline __attribute__((always_inline))                                                \
	RTYPE NAME##_WORK(WorkerP *__lace_worker, Task *__lace_dq_head,                             \
			  ATYPE_1, ATYPE_2, ATYPE_3, ATYPE_4, ATYPE_5);                             \
                                                                                                    \
	/* NAME##_WORK is inlined in NAME##_CALL and the parameter __lace_in_task will disappear */ \
	RTYPE NAME##_CALL(WorkerP *w, Task *__dq_head, ATYPE_1 arg_1,                               \
			  ATYPE_2 arg_2, ATYPE_3 arg_3, ATYPE_4 arg_4,                              \
...
#define TASK_5(RTYPE, NAME, ATYPE_1, ARG_1, ATYPE_2, ARG_2, ATYPE_3, ARG_3,   \
	       ATYPE_4, ARG_4, ATYPE_5, ARG_5)                                \
	TASK_DECL_5(RTYPE, NAME, ATYPE_1, ATYPE_2, ATYPE_3, ATYPE_4, ATYPE_5) \
	TASK_IMPL_5(RTYPE, NAME, ATYPE_1, ARG_1, ATYPE_2, ARG_2, ATYPE_3,     \
		    ARG_3, ATYPE_4, ARG_4, ATYPE_5, ARG_5)
...
#define VOID_TASK_IMPL_5(NAME, ATYPE_1, ARG_1, ATYPE_2, ARG_2, ATYPE_3, ARG_3,                      \
			 ATYPE_4, ARG_4, ATYPE_5, ARG_5)                                            \
	void NAME##_WRAP(WorkerP *w, Task *__dq_head,                                               \
			 TD_##NAME *t __attribute__((unused)))                                      \
	{                                                                                           \
	...
	static inline __attribute__((always_inline)) void NAME##_WORK(                              \
		WorkerP *__lace_worker, Task *__lace_dq_head, ATYPE_1,                              \
		ATYPE_2, ATYPE_3, ATYPE_4, ATYPE_5);                                                \
                                                                                                    \
	/* NAME##_WORK is inlined in NAME##_CALL and the parameter __lace_in_task will disappear */ \
	void NAME##_CALL(WorkerP *w, Task *__dq_head, ATYPE_1 arg_1,                                \
			 ATYPE_2 arg_2, ATYPE_3 arg_3, ATYPE_4 arg_4,                               \
	...
	static inline __attribute__((always_inline)) void NAME##_WORK(                              \
		WorkerP *__lace_worker __attribute__((unused)),                                     \
		Task *__lace_dq_head __attribute__((unused)), ATYPE_1 ARG_1,                        \
		ATYPE_2 ARG_2, ATYPE_3 ARG_3, ATYPE_4 ARG_4, ATYPE_5 ARG_5)

#define VOID_TASK_5(NAME, ATYPE_1, ARG_1, ATYPE_2, ARG_2, ATYPE_3, ARG_3,      \
		    ATYPE_4, ARG_4, ATYPE_5, ARG_5)                            \
	VOID_TASK_DECL_5(NAME, ATYPE_1, ATYPE_2, ATYPE_3, ATYPE_4, ATYPE_5)    \
	VOID_TASK_IMPL_5(NAME, ATYPE_1, ARG_1, ATYPE_2, ARG_2, ATYPE_3, ARG_3, \
			 ATYPE_4, ARG_4, ATYPE_5, ARG_5)

// Task macros for tasks of arity 6

#define TASK_DECL_6(RTYPE, NAME, ATYPE_1, ATYPE_2, ATYPE_3, ATYPE_4, ATYPE_5,                                 \
		    ATYPE_6)                                                                                  \
                                                                                                              \
	typedef struct _TD_##NAME {                                                                           \
		TASK_COMMON_FIELDS(_TD_##NAME)                                                                \
	...
	static inline __attribute__((unused)) void NAME##_SPAWN(                                              \
		WorkerP *w, Task *__dq_head, ATYPE_1 arg_1, ATYPE_2 arg_2,                                    \
		ATYPE_3 arg_3, ATYPE_4 arg_4, ATYPE_5 arg_5, ATYPE_6 arg_6)                                   \
	{                                                                                                     \
		PR_COUNTTASK(w);                                                                              \
                                                                                                              \
		TD_##NAME *t;                                                                                 \
	...
	static inline __attribute__((unused))                                                                 \
	RTYPE NAME##_NEWFRAME(ATYPE_1 arg_1, ATYPE_2 arg_2, ATYPE_3 arg_3,                                    \
			      ATYPE_4 arg_4, ATYPE_5 arg_5, ATYPE_6 arg_6)                                    \
	{                                                                                                     \
		Task _t;                                                                                      \
		TD_##NAME *t = (TD_##NAME *)&_t;                                                              \
		t->f = &NAME##_WRAP;                                                                          \
...
#define TASK_IMPL_6(RTYPE, NAME, ATYPE_1, ARG_1, ATYPE_2, ARG_2, ATYPE_3,                           \
		    ARG_3, ATYPE_4, ARG_4, ATYPE_5, ARG_5, ATYPE_6, ARG_6)                          \
	void NAME##_WRAP(WorkerP *w, Task *__dq_head,                                               \
			 TD_##NAME *t __attribute__((unused)))                                      \
	{                                                                                           \
		t->d.res = NAME##_CALL(w, __dq_head, t->d.args.arg_1,                               \
...
#define TASK_6(RTYPE, NAME, ATYPE_1, ARG_1, ATYPE_2, ARG_2, ATYPE_3, ARG_3,   \
	       ATYPE_4, ARG_4, ATYPE_5, ARG_5, ATYPE_6, ARG_6)                \
	TASK_DECL_6(RTYPE, NAME, ATYPE_1, ATYPE_2, ATYPE_3, ATYPE_4, ATYPE_5, \
		    ATYPE_6)                                                  \
	TASK_IMPL_6(RTYPE, NAME, ATYPE_1, ARG_1, ATYPE_2, ARG_2, ATYPE_3,     \
		    ARG_3, ATYPE_4, ARG_4, ATYPE_5, ARG_5, ATYPE_6, ARG_6)

#define VOID_TASK_DECL_6(NAME, ATYPE_1, ATYPE_2, ATYPE_3, ATYPE_4, ATYPE_5,                                   \
			 ATYPE_6)                                                                             \
                                                                                                              \
	typedef struct _TD_##NAME {                                                                           \
		TASK_COMMON_FIELDS(_TD_##NAME)                                                                \
...
#define VOID_TASK_IMPL_6(NAME, ATYPE_1, ARG_1, ATYPE_2, ARG_2, ATYPE_3, ARG_3,                      \
			 ATYPE_4, ARG_4, ATYPE_5, ARG_5, ATYPE_6, ARG_6)                            \
	void NAME##_WRAP(WorkerP *w, Task *__dq_head,                                               \
			 TD_##NAME *t __attribute__((unused)))                                      \
	{                                                                                           \
		NAME##_CALL(w, __dq_head, t->d.args.arg_1, t->d.args.arg_2,                         \
			    t->d.args.arg_3, t->d.args.arg_4, t->d.args.arg_5,                      \
			    t->d.args.arg_6);                                                       \
	}                                                                                           \
                                                                                                    \
	static inline __attribute__((always_inline)) void NAME##_WORK(                              \
		WorkerP *__lace_worker, Task *__lace_dq_head, ATYPE_1,                              \
		ATYPE_2, ATYPE_3, ATYPE_4, ATYPE_5, ATYPE_6);                                       \
                                                                                                    \
	/* NAME##_WORK is inlined in NAME##_CALL and the parameter __lace_in_task will disappear */ \
	void NAME##_CALL(WorkerP *w, Task *__dq_head, ATYPE_1 arg_1,                                \
			 ATYPE_2 arg_2, ATYPE_3 arg_3, ATYPE_4 arg_4,                               \
...
#define VOID_TASK_6(NAME, ATYPE_1, ARG_1, ATYPE_2, ARG_2, ATYPE_3, ARG_3,      \
		    ATYPE_4, ARG_4, ATYPE_5, ARG_5, ATYPE_6, ARG_6)            \
	VOID_TASK_DECL_6(NAME, ATYPE_1, ATYPE_2, ATYPE_3, ATYPE_4, ATYPE_5,    \
			 ATYPE_6)                                              \
	VOID_TASK_IMPL_6(NAME, ATYPE_1, ARG_1, ATYPE_2, ARG_2, ATYPE_3, ARG_3, \
			 ATYPE_4, ARG_4, ATYPE_5, ARG_5, ATYPE_6, ARG_6)
