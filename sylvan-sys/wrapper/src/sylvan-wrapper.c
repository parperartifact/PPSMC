...
void Sylvan_set_sizes(size_t min_tablesize, size_t max_tablesize,
		      size_t min_cachesize, size_t max_cachesize)
{
	sylvan_set_sizes(min_tablesize, max_tablesize, min_cachesize,
			 max_cachesize);
}
...
void Sylvan_mt_set_write_binary(uint32_t type,
				sylvan_mt_write_binary_cb write_binary_cb)
{
	sylvan_mt_set_write_binary(type, write_binary_cb);
}

void Sylvan_mt_set_read_binary(uint32_t type,
...
char *Sylvan_mt_to_str(int complement, uint32_t type, uint64_t value, char *buf,
		       size_t buflen)
{
...
MDD Sylvan_lddmc_union_cube_copy(MDD a, uint32_t *values, int *copy,
				 size_t count)
{
	return lddmc_union_cube_copy(a, values, copy, count);
}

int Sylvan_lddmc_member_cube_copy(MDD a, uint32_t *values, int *copy,
				  size_t count)
{
...
void Sylvan_lddmc_match_sat_par(MDD mdd, MDD match, MDD proj, lddmc_enum_cb cb,
				void *context)
{
...
void Sylvan_lddmc_visit_par(MDD mdd, lddmc_visit_callbacks_t *cbs,
			    size_t ctx_size, void *context)
{
	lddmc_visit_par(mdd, cbs, ctx_size, context);
}

void Sylvan_lddmc_visit_seq(MDD mdd, lddmc_visit_callbacks_t *cbs,
			    size_t ctx_size, void *context)
{
...
BDD Sylvan_mtbdd_union_cube(MTBDD mtbdd, MTBDD variables, uint8_t *cube,
			    MTBDD terminal)
{
...
MTBDD Sylvan_mtbdd_applyp(MTBDD a, MTBDD b, size_t p, mtbdd_applyp_op op,
			  uint64_t opid)
{
...
MTBDD Sylvan_mtbdd_enum_first(MTBDD dd, MTBDD variables, uint8_t *arr,
			      mtbdd_enum_filter_cb filter_cb)
{
	return mtbdd_enum_first(dd, variables, arr, filter_cb);
}

MTBDD Sylvan_mtbdd_enum_next(MTBDD dd, MTBDD variables, uint8_t *arr,
			     mtbdd_enum_filter_cb filter_cb)
{
	return mtbdd_enum_next(dd, variables, arr, filter_cb);
}

MTBDD Sylvan_mtbdd_enum_all_first(MTBDD dd, MTBDD variables, uint8_t *arr,
				  mtbdd_enum_filter_cb filter_cb)
{
	return mtbdd_enum_all_first(dd, variables, arr, filter_cb);
}

MTBDD Sylvan_mtbdd_enum_all_next(MTBDD dd, MTBDD variables, uint8_t *arr,
				 mtbdd_enum_filter_cb filter_cb)
{
...
void Sylvan_mtbdd_visit_seq(MTBDD dd, mtbdd_visit_pre_cb pre,
			    mtbdd_visit_post_cb post, void *context)
{
	mtbdd_visit_seq(dd, pre, post, context);
}

void Sylvan_mtbdd_visit_par(MTBDD dd, mtbdd_visit_pre_cb pre,
			    mtbdd_visit_post_cb post, void *context)
{
...
Task *Lace_task_offset(Task *task, int offset)
{
	return task + offset;
}

TASK_2(void *, lace_run_func, void *, func, void *, arg)
{
	void *(*f)(WorkerP *, Task *, void *) =
		(void *(*)(WorkerP *, Task *, void *))func;
	return f(__lace_worker, __lace_dq_head, arg);
}

void *Lace_run_func(void (*f)(WorkerP *, Task *, void *), void *arg)
{
	return RUN(lace_run_func, f, arg);
}

void *Lace_run_func_without_block(void (*f)(WorkerP *, Task *, void *),
				  void *arg)
{
	return RUN_WITHOUT_BLOCK(lace_run_func, f, arg);
}

Task *Lace_spawn_func(WorkerP *__lace_worker, Task *__lace_dq_head,
		      void (*f)(WorkerP *, Task *, void *), void *arg)
{
	return SPAWN(lace_run_func, f, arg);
}

void *Lace_sync_func(WorkerP *__lace_worker, Task *__lace_dq_head)
{
	return SYNC(lace_run_func);
}

void Lace_yield_newframe(WorkerP *__lace_worker, Task *__lace_dq_head)
{
	YIELD_NEWFRAME();
}

void Lace_steal_random(WorkerP *__lace_worker, Task *__lace_dq_head)
{
	STEAL_RANDOM();
}
